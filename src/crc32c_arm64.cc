...
#define CRC32C32BYTES(P, IND)                                             \
  do {                                                                    \
    uint64_t d;                                                           \
    std::memcpy(&d, (P) + SEGMENTBYTES * 1 + (IND) * 8, sizeof(d));        \
    crc1 = __crc32cd(crc1, d);                                            \
    std::memcpy(&d, (P) + SEGMENTBYTES * 2 + (IND) * 8, sizeof(d));        \
    crc2 = __crc32cd(crc2, d);                                            \
    std::memcpy(&d, (P) + SEGMENTBYTES * 3 + (IND) * 8, sizeof(d));        \
    crc3 = __crc32cd(crc3, d);                                            \
    std::memcpy(&d, (P) + SEGMENTBYTES * 0 + (IND) * 8, sizeof(d));        \
    crc0 = __crc32cd(crc0, d);                                            \
  } while (0);
...
uint32_t ExtendArm64(uint32_t crc, const uint8_t *data, size_t size) {
  ...
  if (length & 4) {
    std::memcpy(&d, data, sizeof(d32));
    crc = __crc32cw(crc, d32);
    data += 4;
  }

  if (length & 2) {
    std::memcpy(&d, data, sizeof(d16));
